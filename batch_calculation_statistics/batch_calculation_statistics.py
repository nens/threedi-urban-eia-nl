...
def repetition_time_volumes(weir_results, n, stats=STATS):
    ...
    if n == 10:
        ...
        for T in stats:
            if T==5:
                volume = sorted_weir_results[1] - 0.48 * (sorted_weir_results[1] - sorted_weir_results[2])
                T_volume_list += [volume]
            elif T==10:
                volume = sorted_weir_results[0] - 0.46 * (sorted_weir_results[0] - sorted_weir_results[1])
                T_volume_list += [volume]
            else:
    ...
    if n == 25:
        ...
        for T in stats:
           if (n/T).is_integer():
               ...
               volume = sorted_weir_results[floor(n/T)-1] - 0.5 * (sorted_weir_results[floor(n/T)-1] - sorted_weir_results[floor(n/T)])
               T_volume_list += [volume]   
...
def batch_calculation_statistics(netcdf_dir, gridadmin, nr_years):
    ...
    for i, aggregate_file in enumerate(nc_files):
        ga = GridH5AggregateResultAdmin(gridadmin, os.path.join(netcdf_dir, aggregate_file))
        weir_data = (
...
#    nr_years = 10
    ...
    batch_calculation_stats_table = batch_calculation_statistics(netcdf_dir = nc_dir,
                                                                 gridadmin = gridadmin,
                                                                 nr_years = nr_years)
